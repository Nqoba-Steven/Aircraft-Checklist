...
tmplNACLogin.innerHTML =
    `
        ...
        <div id="container" class="isHidden">
            ...
            <form>
                <div>
                    <input id="username"  type="text" class="fields" style="margin-bottom:32px;background-color:#282727; width:100%;"  lable="username" name="username" placeholder="Username"/>
                </div>
...
class NACLogin extends HTMLElement {
        ...
        if (this.payload.Username.length > 0 && this.payload.Password.length > 0) {
            fetch(this.target, {
                ...
                if (res.ok)
                    window.location = '/';
                if (res.status == 401) {
                    this.shake("Incorrect username or password");
    ...
    static get observedAttributes() {
        return ['is-visible', 'src', 'target', 'timer']
    }
}
